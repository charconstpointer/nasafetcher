...
type MockClient struct {
	tokens     chan struct{}
	jobFakeDur time.Duration
	timeout    time.Duration
}

func NewMockClient(maxConc int, timeout time.Duration, jobFakeDur time.Duration) *MockClient {
	c := MockClient{
		tokens:     make(chan struct{}, maxConc),
		timeout:    timeout,
		jobFakeDur: jobFakeDur,
	}
	for i := 0; i < maxConc; i++ {
		c.tokens <- struct{}{}
	}
...
func (c *MockClient) Get(ctx context.Context, url string) ([]byte, error) {
	select {
	case <-c.tokens:
